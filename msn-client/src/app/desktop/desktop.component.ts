...
export class DesktopComponent implements AfterViewInit, OnDestroy, OnInit {
  ...
  constructor(
    ...
    this._subscriptions.push(
      this._erreurService.erreursEvent$.subscribe((erreur) =>
        this.onErreurReceived(erreur)
      )
    );
  ...
  onOpenApp(
    ...
    if (this._nbBasicWindowOpen >= 2) {
      const erreur = new Erreur(
        'Fermez les programes pour éviter la perte de données',
    ...
    const componentRef =
      this.iframesContainer.createComponent(BasicWindowComponent);
  ...
  onBackgroundChange() {
    if (localStorage.getItem('background')) {
      ...
      this.desktop.nativeElement.style.backgroundImage = `url(${localStorage.getItem(
        'background'
      )})`;
    }
  }
  ...
  onNotificationReceived(notification: Notification) {
    ...
    const componentRef = this.notificationContainer?.createComponent(
  ...
  onCloseNotification(index: number) {
    ...
    if (!componentRef) return;
    const instance = componentRef.instance;
    const indexUpToDate = this.notificationContainer?.indexOf(
  ...
  onTetrisOpenEvent() {
    if (this.tetrisOpened()) {
      ...
      this.onOpenApp(
        'tetris',
        'https://cdn.htmlgames.com/TetrisFun/',
        'Tetris'
      );
    }
  ...
  onCodeOpenEvent() {
    if (this.codeOpened()) {
      ...
      this.onOpenApp(
        'code',
        'https://github1s.com/jxddiss/msn',
        'Visual Studio Code',
        true
      );
    }
